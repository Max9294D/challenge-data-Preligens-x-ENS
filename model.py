...
def UNet(input_shape,
    ...
    def bn_conv_relu(input, filters, bachnorm_momentum, **conv2d_args):
        x = kl.BatchNormalization(momentum=bachnorm_momentum)(input)
        x = kl.Conv2D(filters, **conv2d_args)(x)
        return x
    ...
    def bn_upconv_relu(input, filters, bachnorm_momentum, **conv2d_trans_args):
        x = kl.BatchNormalization(momentum=bachnorm_momentum)(input)
        x = kl.Conv2DTranspose(filters, **conv2d_trans_args)(x)
        return x
    ...
    c1 = bn_conv_relu(x, filters, bachnorm_momentum, **conv2d_kwargs)
    x = bn_conv_relu(c1, filters, bachnorm_momentum, **conv2d_kwargs)
    x = kl.MaxPooling2D(**maxpool2d_kwargs)(x)

    down_layers = []

    for _ in range(num_layers):
        x = bn_conv_relu(x, filters, bachnorm_momentum, **conv2d_kwargs)
        x = bn_conv_relu(x, filters, bachnorm_momentum, **conv2d_kwargs)
    ...
    x = bn_conv_relu(x, filters, bachnorm_momentum, **conv2d_kwargs)
    x = bn_conv_relu(x, filters, bachnorm_momentum, **conv2d_kwargs)
    x = bn_upconv_relu(x, filters, bachnorm_momentum, **conv2d_transpose_kwargs)

    for conv in reversed(down_layers):
        x = kl.concatenate([x, conv])
        x = bn_conv_relu(x, upconv_filters, bachnorm_momentum, **conv2d_kwargs)
        x = bn_conv_relu(x, filters, bachnorm_momentum, **conv2d_kwargs)
        x = bn_upconv_relu(x, filters, bachnorm_momentum, **conv2d_transpose_kwargs)
